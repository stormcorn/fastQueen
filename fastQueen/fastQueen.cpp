...
int main()
{
	...
	int n, fight, run , count = 0;
	cin >> n;
	cout << "\n\n How many puzzles you want?";
	cin >> run;

	auto start = high_resolution_clock::now();//start count time
	for (int k = 0; k < run; k++)
	{
	...
	auto end = high_resolution_clock::now();//end count time
	auto duration = duration_cast<microseconds>(end - start);//time spend
